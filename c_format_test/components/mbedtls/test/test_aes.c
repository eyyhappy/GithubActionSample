...
ret_code_t parse_public_key( mbedtls_pk_context* pk, const uint8_t* raw_public_key, size_t key_length)
{
    ret_code_t ret = 0;
    mbedtls_pk_init(pk);
    // 设置ECC SECP256R1曲线类型
    ...
    if (ret != 0)
    {
        printf("Failed to setup PK context. Error code: %d\n", ret);
        return ret;
    }
    mbedtls_ecp_keypair* ec_key = mbedtls_pk_ec(*pk);
    ret = mbedtls_ecp_group_load(&ec_key->MBEDTLS_PRIVATE(grp), curve_id);
    if (ret != 0)
    {
        printf("Failed to load ECC group. Error code: %d\n", ret);
        return ret;
    }
    ret = mbedtls_ecp_point_read_binary(&ec_key->MBEDTLS_PRIVATE(grp), &ec_key->MBEDTLS_PRIVATE(Q), raw_public_key, key_length);
    if (ret != 0)
    {
        printf("Failed to read ECC public key. Error code: %d\n", ret);
        return ret;
    }
    return ret;
}
...
#define ERROR_SIGNATURE_VERIFICATION_FAILED -3
...
ret_code_t gwp_secp256r1_verify(const uint8_t* raw_public_key, const uint8_t* in_hash, const uint8_t* signature)
{
    ...
    mbedtls_mpi_init(&s);
    printf("init success\r\n");
    // Set the elliptic curve parameters to secp256r1
    mbedtls_ecp_group_init( &keypair.private_grp );
    // Load the curve group
    mbedtls_ecp_group_load(&keypair.private_grp, MBEDTLS_ECP_DP_SECP256R1) ;
    if (ret != 0)
    {
        // keypair.private_Q.private_X
        printf("Failed to load secp256r1 curve: %d\n", ret);
        goto exit;
    }
    printf("load success\r\n");
    mbedtls_ecp_point_init(&keypair.private_Q);
    // Set the public key coordinates
    ret = mbedtls_mpi_read_binary(&keypair.private_Q.private_X, raw_public_key, 32);
    if (ret != 0)
    {
        printf("Failed to set public key X coordinate: %d\n", ret);
        goto exit;
    }
    printf("x success\r\n");
    ret = mbedtls_mpi_read_binary(&keypair.private_Q.private_Y, raw_public_key + 32, 32);
    if (ret != 0)
    {
        printf("Failed to set public key Y coordinate: %d\n", ret);
        goto exit;
    }
    printf("y success\r\n");
    ret = mbedtls_mpi_lset(&keypair.private_Q.private_Z, 1); // Set Z coordinate to 1
    if (ret != 0)
    {
        printf("Failed to set public key Z coordinate: %d\n", ret);
        goto exit;
    }
    printf("z success\r\n");
    // Check that the public key belongs to the curve
    mbedtls_ecp_check_pubkey(&keypair.private_grp, &keypair.private_Q);
    printf("\r\nmbedtls_ecp_check_pubkey pass\r\n");
    // Set the r and s values of the signature
    ret = mbedtls_mpi_read_binary(&r, signature, 32);
    if (ret != 0)
    {
        printf("Failed to read r value: %d\n", ret);
        goto exit;
    }
    printf("read sign 1 success\r\n");
    ret = mbedtls_mpi_read_binary(&s, signature + 32, 32);
    if (ret != 0)
    {
        printf("Failed to read s value: %d\n", ret);
        goto exit;
    }
    printf("read sign 2 success\r\n");
    ret = mbedtls_ecdsa_verify(&keypair.private_grp, in_hash, 32, &keypair.private_Q, &r, &s);
    if (ret != 0)
    {
...
exit:
    mbedtls_ecp_point_free(&keypair.private_Q);
    mbedtls_ecp_group_free(&keypair.private_grp);
    mbedtls_mpi_free(&r);
    mbedtls_mpi_free(&s);
    return ret;
}
...
TEST_CASE("mbedtls ecdsa test", "[ecdsa]")
{
    ret_code_t result = 0;
    uint8_t public_key[] =
    {
        0x37, 0x15, 0xf2, 0xe9, 0x2b, 0xbb, 0x09, 0x53, 0x36, 0x99, 0x74, 0xd1, 0x58, 0x71, 0x39, 0x4d,
        0x9e, 0x5d, 0xa6, 0x0b, 0x4f, 0xd4, 0x4c, 0xe5, 0xe2, 0x63, 0xdb, 0xa6, 0x45, 0xaa, 0xdf, 0x40,
        0xdd, 0x17, 0xdd, 0x5a, 0x9c, 0xed, 0xb6, 0xf4, 0x72, 0xe1, 0x50, 0xa0, 0xa5, 0xd1, 0x32, 0x0a,
        0x04, 0x4e, 0x94, 0xb2, 0x54, 0xaf, 0xdc, 0x56, 0xfb, 0x83, 0x50, 0xa2, 0xde, 0xf3, 0x64, 0x79
    };
    uint8_t digest[32] =//actual is hash
    {
        0xf2, 0xed, 0x57, 0x78, 0xcc, 0xd8, 0x6e, 0x87, 0x02, 0x36, 0x5a, 0xb7, 0xd1, 0x64, 0x84, 0xd3,
        0x2b, 0x7a, 0x5e, 0xc2, 0x61, 0x11, 0x86, 0xb3, 0x47, 0x39, 0x9c, 0xeb, 0x07, 0xff, 0x45, 0x30
    };
    uint8_t signature[64] =
    {

        0x11, 0x60, 0x06, 0xf9, 0xc5, 0xb8, 0x6d, 0xb2, 0x86, 0xc4, 0x8c, 0xbf, 0x80, 0x67, 0x9c, 0x1e,
        0x3c, 0xde, 0x45, 0x92, 0x0c, 0x6b, 0x34, 0x8c, 0xee, 0xd9, 0x1f, 0xad, 0x6f, 0x1b, 0x17, 0x99,
        0x1f, 0x0c, 0xd8, 0xed, 0x39, 0xbf, 0xb4, 0x8d, 0xe6, 0x32, 0xf9, 0x39, 0x0a, 0xb5, 0x58, 0x82,
        0xdb, 0xbd, 0x52, 0x8e, 0x58, 0xde, 0xdc, 0x29, 0x50, 0x5e, 0xe4, 0xe9, 0xc2, 0xf7, 0xd2, 0xc9
    };
    public_key[0] = 0xff;
    result = gwp_secp256r1_verify(public_key, digest, signature);
    printf("result value is %d\r\n", result);
    TEST_ASSERT_NOT_EQUAL_MESSAGE(0, result, "Unexpected error code1 pub key error");
    public_key[0] = 0x37;
    digest[0] = 0xff;
    result = gwp_secp256r1_verify(public_key, digest, signature);
    printf("result value is %d\r\n", result);
    TEST_ASSERT_NOT_EQUAL_MESSAGE(0, result, "Unexpected error code2 digest error");
    public_key[0] = 0x37;
    digest[0] = 0xf2;
    signature[0] = 0xff;
    result = gwp_secp256r1_verify(public_key, digest, signature);
    printf("result value is %d\r\n", result);
    TEST_ASSERT_NOT_EQUAL_MESSAGE(0, result, "Unexpected error code2 signature error");
    public_key[0] = 0x37;
    digest[0] = 0xf2;
    signature[0] = 0x11;
    result = gwp_secp256r1_verify(public_key, digest, signature);
    printf("result value is %d\r\n", result);
    printf("test success.................\r\n");
    TEST_ASSERT_EQUAL_HEX32_MESSAGE(0, result, "Unexpected error code");
}



#include "mbedtls/md.h"
// #include "gwp_hmac256.h"
#define GWP_HMAC256_DIGEST_SIZE 32
ret_code_t gwp_hmac256_calculate(uint8_t const * p_data,
                                 size_t data_size,
                                 uint8_t const * p_key,
                                 size_t key_size,
                                 uint8_t* p_digest,
...
{
    if (p_data == NULL || p_key == NULL || p_digest == NULL || p_digest_size == NULL)
    {
        return 1;
    }
    if (*p_digest_size != GWP_HMAC256_DIGEST_SIZE)
    {
        return 2;
    }
    ...
    mbedtls_md_setup(&context, md_info, 1);
    mbedtls_md_hmac_starts(&context, p_key, key_size);
    mbedtls_md_hmac_update(&context, p_data, data_size);
    mbedtls_md_hmac_finish(&context, p_digest);
    mbedtls_md_free(&context);
    *p_digest_size = GWP_HMAC256_DIGEST_SIZE;
    return 0;
}

TEST_CASE("mbedtls hmac256 test", "[hmac256]")
{
    const uint8_t key2[] =
    {
    ...
    const uint8_t data[] =
    {
        0xe9, 0x28, 0x15, 0x00, 0x51, 0xf2, 0xff, 0x13, 0x23, 0x77, 0x6a, 0x49, 0x6f, 0x36, 0x58, 0xa7,
        0xd4, 0xcd, 0x99, 0xf8, 0x00, 0x00, 0x00, 0x00, 0x01,
    };
    size_t const key_size = sizeof(key2);
    size_t const data_size = sizeof(data);
    uint8_t digest[GWP_HMAC256_DIGEST_SIZE] = {0};
    size_t digest_size = sizeof(digest);
    ret_code_t ret = gwp_hmac256_calculate(data, data_size, key2, key_size, digest, &digest_size);
    TEST_ASSERT_EQUAL_HEX8_MESSAGE(0, ret, "Unexpected error code");
    // Calculate expected digest using an external SHA256 implementation
    uint8_t const expected_digest[] =
    {
        0x0f, 0xed, 0x19, 0x19, 0x77, 0x39, 0x41, 0x1f, 0x53, 0x26, 0xa0, 0xff, 0xae, 0x26, 0xd9, 0x08,
        0xac, 0x56, 0xb9, 0x72, 0xe7, 0x2a, 0x81, 0xbe, 0xc3, 0xbc, 0x4e, 0xf2, 0x1e, 0xda, 0xd5, 0x28,
    };
    print_hex(digest, sizeof(digest), "hmac256 digest value");
    TEST_ASSERT_EQUAL_HEX8_ARRAY_MESSAGE(expected_digest, digest, GWP_HMAC256_DIGEST_SIZE, "Unexpected digest value");
}
...
#define GWP_CRYPTO_ECC_SECP256R1_RAW_PUBLIC_KEY_SIZE   (2 * 256 / 8)
// uint32_t gwp_secp256r1_verify(const uint8_t *raw_public_key, const uint8_t *digest, const uint8_t *signature)
// #include "mbedtls/ecdsa.h"
// #include "mbedtls/error.h"
...
// ret_code_t gwp_secp256r1_verify(const uint8_t* raw_public_key, const uint8_t* digest, const uint8_t* signature)
// {
//     ret_code_t ret = 0;
...
//         goto exit;
//     }
//     if (mbedtls_ecp_point_read_binary(&ctx.grp, &ctx.Q, raw_public_key, 64) != 0) {
//         ret = MBEDTLS_ERR_ECP_BAD_INPUT_DATA;
//         goto exit;
//     }

//     if (mbedtls_ecdsa_verify(&ctx.grp, digest, 32, &ctx.Q, signature, 64) != 0) {
...
//     uint8_t raw_public_key[GWP_CRYPTO_ECC_SECP256R1_RAW_PUBLIC_KEY_SIZE] =
//     {
//         0x37, 0x15, 0xf2, 0xe9, 0x2b, 0xbb, 0x09, 0x53, 0x36, 0x99, 0x74, 0xd1, 0x58, 0x71, 0x39, 0x4d,
//         0x9e, 0x5d, 0xa6, 0x0b, 0x4f, 0xd4, 0x4c, 0xe5, 0xe2, 0x63, 0xdb, 0xa6, 0x45, 0xaa, 0xdf, 0x40,
//         0xdd, 0x17, 0xdd, 0x5a, 0x9c, 0xed, 0xb6, 0xf4, 0x72, 0xe1, 0x50, 0xa0, 0xa5, 0xd1, 0x32, 0x0a,
...
//     uint32_t ret_val=0;
//     uint8_t digest[32] = {0xf2, 0xed, 0x57, 0x78, 0xcc, 0xd8, 0x6e, 0x87, 0x02, 0x36, 0x5a, 0xb7, 0xd1, 0x64, 0x84, 0xd3,
//                           0x2b, 0x7a, 0x5e, 0xc2, 0x61, 0x11, 0x86, 0xb3, 0x47, 0x39, 0x9c, 0xeb, 0x07, 0xff, 0x45, 0x30
//                          };
//     uint8_t signature[] = {0x11, 0x60, 0x06, 0xf9, 0xc5, 0xb8, 0x6d, 0xb2, 0x86, 0xc4, 0x8c, 0xbf, 0x80, 0x67, 0x9c, 0x1e,
//                            0x3c, 0xde, 0x45, 0x92, 0x0c, 0x6b, 0x34, 0x8c, 0xee, 0xd9, 0x1f, 0xad, 0x6f, 0x1b, 0x17, 0x99,
//                            0x1f, 0x0c, 0xd8, 0xed, 0x39, 0xbf, 0xb4, 0x8d, 0xe6, 0x32, 0xf9, 0x39, 0x0a, 0xb5, 0x58, 0x82,
//                            0xdb, 0xbd, 0x52, 0x8e, 0x58, 0xde, 0xdc, 0x29, 0x50, 0x5e, 0xe4, 0xe9, 0xc2, 0xf7, 0xd2, 0xc9
//                           };
//     ret_val = gwp_secp256r1_verify(raw_public_key, digest, signature);
//     TEST_ASSERT_EQUAL_HEX32_ARRAY(ret_val, true, 1);

// }



ret_code_t gwp_hkdf_calculate(uint8_t *output,
                              ...
                              uint8_t info_size)
{
    ...
    int ret = mbedtls_hkdf(md, salt, salt_size, input_key, input_size, info, info_size, output, output_size);
    if (ret != 0)
    {
        return -1;
    }
...
const uint8_t rpc_session_key_expect[32] =
{
    0xdf, 0x5f, 0xb7, 0xfe, 0x71, 0x36, 0x5d, 0x4d, 0x95, 0xba, 0x26, 0xe7, 0xb1, 0x73, 0x58, 0x18,
    0x23, 0xa8, 0xd7, 0xc6, 0x8b, 0x0d, 0x9b, 0x56, 0x4d, 0x04, 0x55, 0xaa, 0x45, 0xfd, 0xde, 0x64
};
...
const uint8_t add_info_ssion[] = "GWP_RPC_INIT";

TEST_CASE("mbedtls hkdf test", "[hkdf]")
{
    memset(rpc_session_key, 0, sizeof(rpc_session_key));
    err_code =  gwp_hkdf_calculate(  rpc_session_key,                          // Output key
    ...
    printf("err code value is %02x\r\n", err_code);
    // TEST_ASSERT_NULL(err_code);    TEST_ASSERT_EQUAL_HEX8_MESSAGE(0, err_code, "Unexpected error code");
    TEST_ASSERT_EQUAL_HEX8_ARRAY(rpc_session_key, rpc_session_key_expect, sizeof(rpc_session_key));
    // TEST_ASSERT_NOT_NULL(plaintext);
    // TEST_ASSERT_NOT_NULL(decryptedtext);
}
TEST_CASE("rng test", "[rng]")
{
#include "esp_random.h"
    uint8_t raw_rng[64] = {0};
    esp_fill_random(raw_rng, sizeof(raw_rng));
    print_hex(raw_rng, sizeof(raw_rng), "random generate");
    // TEST_ASSERT_EQUAL_HEX32_ARRAY(ret_val, true, 1);
}
...
TEST_CASE("mbedtls CTR AES-128 test", "[aes]")
{
    esp_log_level_set("TAG", ESP_LOG_DEBUG);
    mbedtls_aes_context ctx;
    unsigned char key[] =   {0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff, 0x00 };
    unsigned char iv[] =  {0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff, 0x00 };
    unsigned char plaintext1[32] = "This is a test message."; //"Hello, AES 128 CTR Mode!";
    const unsigned SZ = sizeof(plaintext1);
    uint8_t nonce[16];
    uint8_t stream_block[16];
    size_t nc_off = 0;
    const uint8_t expected_cipher_end[] =
    {
        0x5c, 0x52, 0xe2, 0x77, 0x6c, 0x53, 0x93, 0xa9, 0x8a, 0xa2, 0xdd, 0xd9, 0xa7, 0x50, 0xc6, 0x23,
        0x3f, 0xf8, 0xb5, 0xf9, 0x28, 0x9c, 0x35, 0x77, 0x30, 0x04, 0x72, 0x80, 0xc8, 0xe1, 0x73, 0x04
    };
    memcpy(nonce, iv, 16);
    // allocate internal memory
    uint8_t *chipertext = heap_caps_malloc(SZ, MALLOC_CAP_8BIT | MALLOC_CAP_INTERNAL);
    ...
    memset(decryptedtext, 0x0, SZ);
    // Encrypt
    mbedtls_aes_crypt_ctr(&ctx, SZ, &nc_off, nonce, stream_block, plaintext1, chipertext);
    ESP_LOG_BUFFER_HEXDUMP("expected", expected_cipher_end, SZ, ESP_LOG_DEBUG);
    ...
    memcpy(nonce, iv, 16);
    mbedtls_aes_crypt_ctr(&ctx, SZ, &nc_off, nonce, stream_block, chipertext, decryptedtext);
    TEST_ASSERT_EQUAL_HEX8_ARRAY(plaintext1, decryptedtext, SZ);
    mbedtls_aes_free(&ctx);
    // free(plaintext);
    free(chipertext);
    free(decryptedtext);
...
TEST_CASE("mbedtls ccm AES-128 test", "[aes]")
{
    const unsigned SZ = 64;
    unsigned char tag[16] = {0}; //mac?
    // Initialize the context structure
    mbedtls_ccm_context ctx;
    uint8_t nonce[13];
    // size_t nc_off = 0;
    unsigned char key[16] = {0x00};
    unsigned char iv[13] = {0};// nonce
    unsigned char add[16] = {"HoneywellGWP"};
    const uint8_t expected_cipher_end[] =
    {
        0xf4, 0xce, 0x40, 0xdc, 0x82, 0x7e, 0x58, 0xd0, 0x3b, 0x19, 0x80, 0xa9, 0x01, 0x08, 0x08, 0x87,
        0x88, 0x5a, 0x35, 0x51, 0x6d, 0x91, 0x30, 0x1f, 0x2d, 0x08, 0x14, 0x03, 0x38, 0x15, 0x52, 0xc6,
        0x96, 0x5b, 0x29, 0x10, 0xa9, 0xaa, 0x15, 0x1d, 0x9a, 0x04, 0x3f, 0x62, 0x7f, 0x1e, 0x04, 0xaf,
        0xd9, 0x3e, 0x5a, 0x9e, 0x0a, 0xc4, 0xee, 0xb8, 0x47, 0x24, 0x3a, 0x1d, 0xf4, 0x52, 0x9b, 0x5a,
    };
    // allocate internal memory
    uint8_t *chipertext = heap_caps_malloc(SZ, MALLOC_CAP_8BIT | MALLOC_CAP_INTERNAL);
    uint8_t *plaintext = heap_caps_malloc(SZ, MALLOC_CAP_8BIT | MALLOC_CAP_INTERNAL);
    ...
    // Encrypt
    mbedtls_ccm_encrypt_and_tag(&ctx, SZ, nonce, sizeof(nonce), add, sizeof(add), plaintext, chipertext, tag, sizeof(tag));
    TEST_ASSERT_EQUAL_HEX8_ARRAY(expected_cipher_end, chipertext + SZ - 64, 64);
    // Decrypt
    mbedtls_ccm_auth_decrypt(&ctx, SZ, nonce, sizeof(nonce), add, sizeof(add), chipertext, decryptedtext, tag, sizeof(tag));
    TEST_ASSERT_EQUAL_HEX8_ARRAY(plaintext, decryptedtext, SZ);
...
static void aes_ctr_stream_test(void)
{
    ...
    /* Cipher produced via this Python:
        ...
        encryptor = cipher.encryptor()        ct = encryptor.update(b'\xaa' * 100) + encryptor.finalize()
        ct_arr = ""
...
TEST_CASE("mbedtls OFB stream test", "[aes]")
{
    ...
    /* Cipher produced via this Python:
        ...
        key = b'\x44' * 16
        iv = b'\xee' * 16
        cipher = Cipher(algorithms.AES(key), modes.OFB(iv), backend=default_backend())        encryptor = cipher.encryptor()
    ...
    */
    ...
    mbedtls_aes_setkey_enc(&ctx, key, 128);
    memset(plaintext, 0xAA, SZ);
    /* Test that all the end results are the same        no matter how many bytes we encrypt each call
        */
    for (int bytes_to_process = 1; bytes_to_process < SZ; bytes_to_process++)
    {
        ESP_LOGD("test", "bytes_to_process %d", bytes_to_process);
...
TEST_CASE("mbedtls CTR, input buf = output buf", "[aes]")
{
    ...
    const uint8_t expected_cipher_end[] =
    {
        0xd4, 0xdc, 0x4f, 0x8f, 0xfe, 0x86, 0xee, 0xb5,
        0x14, 0x7f, 0xba, 0x30, 0x25, 0xa6, 0x7f, 0x6c,
        0xb5, 0x73, 0xaf, 0x90, 0xd7, 0xff, 0x36, 0xba,        0x2b, 0x1d, 0xec, 0xb9, 0x38, 0xfa, 0x0d, 0xeb,
    };
    memcpy(nonce, iv, 16);
    // allocate internal memory
...
TEST_CASE("mbedtls OFB, chained DMA descriptors", "[aes]")
{
    ...
    uint8_t nonce[16];
    size_t nc_off = 0;
    const uint8_t expected_cipher_end[] = {        0xfe, 0xfa, 0xc9, 0x26, 0xb5, 0xc9, 0xea, 0xb0,
...
void aes_ctr_alignment_test(uint32_t input_buf_caps, uint32_t output_buf_caps)
{
    ...
    memset(key, 0x1E, 16);    // allocate memory according the requested caps
...
const uint8_t long_input[] =
{
    ...
    0xde, 0x75, 0xab, 0x90, 0xb7, 0x75, 0xe9, 0xa0,    0xa9, 0xb0, 0xac, 0x61, 0x5, 0x6d, 0x9a, 0xe3,
    ...
    0x3c, 0x4d, 0xd5, 0xd7, 0x6c, 0xd2, 0x36, 0xa4,    0x22, 0x8, 0x66, 0x48, 0x31, 0xb4, 0x3d, 0xc2,
    0xf6, 0x6b, 0xce, 0xf0, 0x12, 0xe4, 0x38, 0x5c,
    0xd8, 0x71, 0xea, 0x30, 0x52, 0xdf, 0x34, 0x62,
    0xdc, 0xb4, 0x30, 0xe, 0x74, 0xc, 0x5, 0x14,
    0xf, 0x47, 0x25, 0x5, 0x72, 0xc9, 0x14, 0x7c,    0x1f, 0x6e, 0xdb, 0x6f, 0x83, 0x6, 0xa0, 0xb2,
    ...
    0xc6, 0x9c, 0x24, 0xd0, 0x53, 0xc4, 0xfc, 0xed,    0x8e, 0x1d, 0xf, 0xc3, 0x86, 0x9, 0x3d, 0x1d,
    ...
    0xec, 0xe2, 0x5e, 0xc8, 0xe3, 0x64, 0x29, 0x92,    0xd, 0x20, 0x34, 0x9f, 0x19, 0x6e, 0x85, 0xf8,
    ...
    0x1, 0x32, 0x5f, 0xaa, 0x5b, 0x1b, 0xe4, 0x87,    0xec, 0x90, 0x45, 0xc7, 0xc5, 0x6c, 0x11, 0x83,
    0x95, 0xab, 0xdd, 0x71, 0x69, 0x24, 0xc, 0x5c,
    0xc0, 0xf3, 0xc1, 0xb0, 0x5e, 0x1, 0x5e, 0x4,
    0xa1, 0x6e, 0x6e, 0x7d, 0x3f, 0x6f, 0xbd, 0x5d,    0x9, 0x8f, 0x23, 0x53, 0x74, 0x4b, 0xa9, 0x53,    0xd2, 0x10, 0xa1, 0xc0, 0x8e, 0x18, 0xa, 0x2f,
    ...
    0xab, 0x44, 0xa3, 0xcc, 0x1a, 0xaa, 0x23, 0x4e,    0x58, 0x37, 0x7b, 0x11, 0x14, 0xc2, 0xf1, 0xc9,
    ...
    0xff, 0x2, 0xe, 0xd8, 0x1a, 0x8b, 0x3c, 0xcc,    0x58, 0x27, 0x71, 0x2d, 0xb, 0x11, 0x9f, 0x6,
    ...
    0x18, 0x42, 0x6f, 0x8f, 0x96, 0x3d, 0x2b, 0x28,    0xd5, 0x53, 0x62, 0xdd, 0x6b, 0xd0, 0xf8, 0x2e,
    ...
    0x93, 0x7, 0x44, 0xbe, 0xbe, 0x60, 0x23, 0xc1,    0xec, 0x5c, 0xf6, 0x93, 0x38, 0xf9, 0x89, 0x0,
...
TEST_CASE("mbedtls AES stack in RTC RAM", "[mbedtls]")
{
    ...
    TEST_ASSERT_NOT_NULL(xTaskCreateStatic(aes_ctr_stream_test_task, "aes_ctr_task", STACK_SIZE, NULL,                                           3, rtc_stack, &rtc_task));
